...
describe('external model config URL parser', () => {
  ...
  it('also accepts raw JSON in the provider parameter', () => {
    const url = new URL('https://example.com/zh/?provider={"type":"image","preset":"gpt-image-2","provider":"openai","name":"Lynwu","modelId":"gpt-image-2","baseUrl":"https://lynwu.com","apiKey":"raw-key"}');
  ...
  it('removes external config params and hash from URL', () => {
    const provider = encodeURIComponent(JSON.stringify({ type: 'image', name: 'Lynwu', apiKey: 'secret' }));
  ...
  it('keeps legacy multi-param URLs parseable', () => {
    const url = new URL('https://example.com/zh/?configureModel=1&type=image&preset=gpt-image-2&protocol=openai&name=Lynwu&modelId=gpt-image-2&baseUrl=https%3A%2F%2Flynwu.com&apiKey=query-key&maxRefImages=16&maxOutputSize=4K');
